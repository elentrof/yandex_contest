...
# поэтому продажа билетов шла очень медленно, приводя «постояльцев» очереди в отчаяние. Самые сообразительные быстро заметили, что, как правило, 
...
if n == 1:
  ...
  # минимум из покупки каждого по одному билету и если первый купит 2 билета
  d.append(min(one[0] + one[1], two[0]))

  
  for i in range(3, n+1):
    k = min(d[i-1] + one[i-1], d[i-2] + two[i-2])
    d.append(min(k, d[i-3] + three[i-3]))
